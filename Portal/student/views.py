...
def upload_report_file(request, id):
    report_grades = get_object_or_404(Report_student, id=id)

    if str(report_grades.student) != str(request.user):
        raise Http404()

    report = get_object_or_404(Report, description_report=report_grades.report)

    is_deadline_not_over = report.deadline > datetime.datetime.now(timezone.utc)
