...
def teacher_dashboard(request):
    ...
    try:
        ...
        for c in courses:
            for r in all_rooms:
                if str(c) == str(r):
                    is_deadline_not_over = r.meeting_date > datetime.datetime.now(timezone.utc) - datetime.timedelta(
                        hours=0)
...
def course_detail(request, name):
    ...
    materials = Subject.objects.filter(teacher=teacher, course=course).all()
    reports = Report.objects.filter(teacher=teacher, course=course).all()
    if request.method == 'POST':
        add_lec_form = AddMaterialForm(request.POST, request.FILES)
        if add_lec_form.is_valid() and (
                bool(str(request.POST.get('description')).strip()) and str(request.POST.get('description')) != "None"):
    ...
    if q != '':
        try:
            ...
            msg = "Student added to the course successfully"
    ...
    if request.method == 'POST':
        ...
        if add_report_form.is_valid() and (bool(str(request.POST.get('description_report')).strip()) and str(
                request.POST.get('description_report')) != "None"):
            ...
            try:
                ...
                if (students[0]['student'] != None):
                    ...
                    msg = "There are no students in the course. Please add students first."
                for i in students:
                    student_name = Student.objects.get(id=i['student'])
                    report_name = Report.objects.get(description_report=temp.description_report)
                    student_in_quiz = Report_student(student=student_name, course=course, report=report_name,
                                                     grade='Not submitted')
...
def add_course(request):
    ...
    if request.method == 'POST':
        ...
        if course_form.is_valid():
            ...
            msg = 'Error in the form submission!'
            messages.error(request, msg)
            return render(request, 'teacher/add_course.html', {'course_form': course_form, 'msg': msg})
    else:
        course_form = AddCourseForm()
    return render(request, 'teacher/add_course.html', {'course_form': course_form})
...
def report_grades_update(request, id):
    ...
    # check if teacher is the course owner or not
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if report_grades.grade != 'Not submitted':
                new_form.done = True
            else:
                new_form.done = False
            new_form.save()
...
def schedule_meeting(request, name):
    ...
    if request.method == 'POST':
        ...
        meeting_date = datetime.datetime.strptime(form['meeting_date'].value() + ":00.000-0000",
                                                  '%Y-%m-%dT%H:%M:%S.%f%z')
